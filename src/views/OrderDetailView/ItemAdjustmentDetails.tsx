  ...
  return (
    <details className="rounded-medium border border-divider bg-content1">
      <summary className="cursor-pointer p-3 text-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary">
        ...
        {rows.length > 0 && <span className="ms-2 text-xs text-default-500">{t(`orderRefunds.status.${status}`)} · {formatPrice(statusRows.reduce((total, row) => total + row.amount, 0))}{method && ` · ${t(`orderRefunds.method.${method}`)}`}</span>}
      </summary>
