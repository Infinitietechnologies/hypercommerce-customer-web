...
export default function EnhancedSellerMarketing() {
  ...
  return (
    <div className="w-full">
      {/* Hero Section */}
      <div className="relative overflow-hidden rounded-large bg-content1 border border-divider pb-56 sm:pb-64 md:pb-20 bg-[url('/seller-landing/hero-seller-mobile.png')] md:bg-[url('/seller-landing/hero-seller.png')] bg-contain bg-no-repeat bg-bottom md:bg-right">
        <div className="relative max-w-7xl mx-auto px-4 pt-8 md:pt-10">
          <Image
            src={webSettings?.siteHeaderDarkLogo || SELLER_HERO_LOGO_PLACEHOLDER}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 -mt-14 sm:-mt-20 lg:-mt-14">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 bg-content1 rounded-large border border-divider shadow-md divide-y sm:divide-y-0 sm:divide-x divide-divider">
          {heroFeatures.map((feature, idx) => (
            <div key={idx} className="flex items-start gap-3 p-5">
              <div className="shrink-0 w-9 h-9 rounded-full bg-primary-50 text-primary flex items-center justify-center">
                {feature.icon}
              </div>
              <div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 lg:gap-12 items-center">
            <Image
              ...
              radius="lg"
              removeWrapper
              className="w-full h-64 md:h-full md:max-h-96 object-cover"
            />

            <ol className="space-y-4">
              {steps.map((item, idx) => (
                <li key={idx} className="relative flex items-stretch gap-4">
                  ...
                  <div className="flex-1 flex items-start gap-3 rounded-large border border-primary-200 bg-content1 p-4 shadow-sm">
                    <div className="shrink-0 w-9 h-9 rounded-medium bg-primary-50 text-primary flex items-center justify-center">
                      {item.icon}
                    </div>
                    <div>
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((testimonial, idx) => (
              <div
                key={idx}
                className="h-full flex flex-col justify-between gap-6 rounded-large border border-primary-200 bg-primary-50/40 p-5"
              >
                <div>
