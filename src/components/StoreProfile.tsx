...
const StoreProfile: React.FC<StoreProfileProps> = ({ store }) => {
  ...
  const rating = parseFloat(String(store.avg_store_rating)).toFixed(1) || "0.0";
  const lat = store.latitude;
  const lng = store.longitude;
  // Market model has no open/closed; treat as open when status is absent.
  const isOpen = store.status ? store.status.is_open : true;
  ...
  const getStatusText = () => {
    if (!isOpen) return t("closed") || "Closed";
    return store.status?.status === "online" ? "Open Now" : "Offline";
  };
