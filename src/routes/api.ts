...
// Stores (market-scoped; hypercommerce). lat/lng are accepted by the
// backend for backward-compat but ignored — market drives visibility.
...
// Home layout (hypercommerce section/widget builder).
// Replaces the old /featured-sections endpoints. Returns ordered, market-
// visible sections (hero | banners | products | categories | brands) with
// their content materialised.
...
// Paginated content for a single home section ("view all" / load more).
...
): Promise<PaginatedResponse<HomeSectionItem[]>> => {
  try {
...
/* ---------------- Markets (hypercommerce) ---------------- */
// Market is auto-detected server-side (X-Market header → ?market → user
// pivot → market cookie → CDN country header → default). These power the
// optional market/currency switcher; browsing works without calling them.
export const getMarkets = async (): Promise<ApiResponse<Market[]>> => {
  try {
    const response = await api.get<ApiResponse<Market[]>>("/markets");
    return response.data;
  } catch (error) {
    console.error("API error:", error);
    return fallbackApiRes;
...
export const getCurrentMarket = async (): Promise<ApiResponse<Market>> => {
  try {
    const response = await api.get<ApiResponse<Market>>("/markets/current");
    return response.data;
  } catch (error) {
    console.error("API error:", error);
    return fallbackApiRes;
...
export const switchMarket = async (
  ...
  try {
    const response = await api.post<ApiResponse<Market>>("/markets/switch", {
      code,
    });
    return response.data;
  } catch (error) {
...
export const geoDetect = async (): Promise<ApiResponse<GeoDetectData>> => {
  try {
    const response = await api.get<ApiResponse<GeoDetectData>>("/geo-detect");
    return response.data;
  } catch (error) {
    console.error("API error:", error);
    return fallbackApiRes;
