...
export const useInfiniteData = <T>({
  ...
  const {
    ...
    async ([, params]: [string, string]) => {
      ...
      // hypercommerce: market is auto-detected server-side, so browsing no
      // longer requires a location. We still forward any saved lat/lng for
      // backward-compat (the backend ignores it) but never gate on it.
      const { lat = "", lng = "" } =
        (getCookie("userLocation") as UserLocation) || {};

      const location =
        passLocationRef.current && lat && lng
